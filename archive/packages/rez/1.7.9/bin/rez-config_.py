...
p.add_option("-x", "--no-local", dest="no_local", action="store_true", default=False, \
...
# hide local pkgs
if platform.system()=='Windows' and opts.no_local:
	localpath = os.getenv("REZ_LOCAL_PACKAGES_PATH").strip().replace('\\', '/')
	if localpath:
		pkgpaths = os.getenv("REZ_PACKAGES_PATH","").strip().split(';')
		pkgpaths = [ p.replace('\\', '/') for p in pkgpaths]
 		
		if localpath in pkgpaths:
			pkgpaths.remove(localpath)
			os.environ["REZ_PACKAGES_PATH"] = str(';').join(pkgpaths)			

print 'rez package path', os.environ['REZ_PACKAGES_PATH']
